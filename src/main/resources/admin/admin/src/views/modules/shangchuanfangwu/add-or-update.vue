<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'" label="房屋编号" prop="fangwubianhao">
            <el-input v-model="ruleForm.fangwubianhao" 
                placeholder="房屋编号" readonly></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.fangwubianhao" label="房屋编号" prop="fangwubianhao">
              <el-input v-model="ruleForm.fangwubianhao" 
                placeholder="房屋编号" readonly></el-input>
          </el-form-item>
        </div>
      ...
      <el-col :span="24">  
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.xiangguantupian" label="相关图片" prop="xiangguantupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.xiangguantupian.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="房租" prop="fangzu">
          <el-input v-model="ruleForm.fangzu" 
              placeholder="房租" clearable  :readonly="ro.fangzu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="房租" prop="fangzu">
              <el-input v-model="ruleForm.fangzu" 
                placeholder="房租" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="出租方式" prop="chuzufangshi">
          <el-select v-model="ruleForm.chuzufangshi" placeholder="请选择出租方式">
            <el-option
                v-for="(item,index) in chuzufangshiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="出租方式" prop="chuzufangshi">
	      <el-input v-model="ruleForm.chuzufangshi"
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.fangxing = true;
          ...
          if(o=='chuzufangshi'){
            this.ruleForm.chuzufangshi = obj[o];
	    this.ro.chuzufangshi = true;
            continue;
          }
          if(o=='gerenzhanghao'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          var json = data.data;
		if(json.gerenzhanghao!=''&&json.gerenzhanghao){
              		this.ruleForm.gerenzhanghao = json.gerenzhanghao
		}
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `shangchuanfangwu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.shangchuanfangwuCrossAddOrUpdateFlag = false;
                  this.parent.search();
